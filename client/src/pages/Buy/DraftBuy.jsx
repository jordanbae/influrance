...
import styled from "./DraftBuy.module.scss";
...
export default function DraftBuy() {
  ...
  const [chosenPackage, setChosenPackage] = React.useState("");
  const [dOrderId, setdOrderId] = React.useState(0);
  // const chosenPackage = .state.package;
  const navigate = useNavigate();
  React.useEffect(() => {
    axios
      .post(`http://localhost:8085/getnextorder`)
      .then((res) => {
        console.log("nextid", res.data);
        setdOrderId(res.data.nextId);
        setFormData({ ...formData, order_id: res.data.nextId });
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  React.useEffect(() => {
    axios
      .get(`http://localhost:8085/getdraft/${location.state}`)
      .then((res) => {
        console.log("State Id", res);
        setFormData(res.data);
        setChosenPackage(res.data.product_id);
        console.log("New chosen Package", chosenPackage);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(`http://localhost:8085/updatebuy/${formData.id}`, updatedFormData)
      .then((res) => {
        console.log("Buy", res);
        navigate("/");
      })
    ...
    axios
      .post(`http://localhost:8085/agent-customer`, relData)
      .then((res) => {
